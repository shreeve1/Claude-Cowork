...
> Clone this repo from `https://github.com/shreeve1/Claude-Cowork` (into a local temp dir — cloning into the mounted workspace fails). Then copy each folder from its `.claude/skills/` into my `.claude/skills/` directory and each agent from its `agents-library/` into my `.claude/agents/` directory. Also copy its entire `agent-personas/` folder (including `INDEX.json`, `INDEX.md`, and every category subfolder) into my workspace root, as a sibling of `.claude/` — the `professional-guidance` skill reads personas from there. Create any missing folders and ignore `.DS_Store`.
>
